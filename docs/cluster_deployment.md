...
- **监控**：
  ...
  - **紧急清仓**：在非暂停状态下可用，会向协调器发出清仓指令；所有 Agent 会暂停普通循环，按照做市时的随机数量规则反向挂出限价单（使用更激进的对手价偏移）逐步削减库存，直到仓位回到 `flatten_tolerance` 定义的容忍范围内，随后自动保持暂停。默认会持续尝试直至满足该容忍度；若确需限制尝试次数，可在协调器下发的 `metadata` 中额外传入正整数 `max_iterations`。
