#!/usr/bin/env python3
"""GRVT–Lighter hedging cycle executor.

This module orchestrates a four-leg hedging cycle between GRVT and Lighter:
    1. GRVT maker entry.
    2. Lighter opposite taker fill.
    3. GRVT reverse maker exit.
...
class HedgingCycleExecutor:
    """Coordinates the four-leg hedging cycle between GRVT and Lighter."""

    def __init__(self, config: CycleConfig):
        self.config = config
        ticker_label = f"{config.grvt_ticker}_{config.lighter_ticker}".replace("/", "-")
    ...
    async def setup(self) -> None:
        ...
        self.logger.log(
            f"Configured leg quantities -> GRVT: {self.config.grvt_quantity}, Lighter: {self.config.lighter_quantity}",
            "INFO",
        )
    ...
    async def execute_cycle(self) -> List[LegResult]:
        ...
        if not self.grvt_client or not self.lighter_client:
            raise RuntimeError("Exchange clients are not initialized. Call setup() first.")

        results: List[LegResult] = []

        # Leg 1: GRVT maker entry
        entry_direction = self.config.direction
        leg1 = await self._execute_grvt_maker(leg_name="LEG1", direction=entry_direction)
        results.append(leg1)
        ...
        # Leg 3: GRVT reverse maker to flatten
        reverse_direction = "sell" if entry_direction == "buy" else "buy"
        leg3 = await self._execute_grvt_reverse_maker(
    ...
    async def _execute_grvt_maker(self, leg_name: str, direction: str) -> LegResult:
        ...
        while True:
            ...
            order_result = await self.grvt_client.place_open_order(
                self.grvt_config.contract_id, self.config.grvt_quantity, direction
            )
            if not order_result.success:
                last_error = order_result.error_message or "Unknown error"
                self.logger.log(
                    f"{leg_name} | Attempt {attempt} failed to place GRVT order: {last_error}",
                    "ERROR",
                )
                if attempt >= self.config.max_retries:
                    raise RuntimeError(
                        f"{leg_name} | Failed to place GRVT order after {attempt} attempts: {last_error}"
                    )
                continue

            if not order_result.order_id:
                raise RuntimeError(f"{leg_name} | GRVT order returned without an order id")
        ...
        self.logger.log(
            f"{leg_name} | GRVT maker filled {fill_info.filled_size} @ {fill_info.price}",
            "INFO",
        )
    ...
    async def _execute_grvt_reverse_maker(
        self, leg_name: str, direction: str
    ) -> LegResult:
        if not self.grvt_client:
            raise RuntimeError("GRVT client is not connected")

        reverse_quantity = await self.grvt_client.get_account_positions()
        if reverse_quantity <= 0:
            self.logger.log(
                f"{leg_name} | No open position detected on GRVT; falling back to configured quantity {self.config.grvt_quantity}",
                "WARNING",
            )
            reverse_quantity = self.config.grvt_quantity
        else:
            self.logger.log(
                f"{leg_name} | Using current GRVT position {reverse_quantity} as close quantity",
                "INFO",
            )
        ...
        while True:
            ...
            try:
                target_price = await self._calculate_grvt_maker_price(direction)
            except Exception as exc:  # pragma: no cover - defensive logging
                last_error = str(exc)
                self.logger.log(
                    f"{leg_name} | Failed to calculate GRVT price on attempt {attempt}: {last_error}",
                    "ERROR",
                )
                if attempt >= self.config.max_retries:
                    raise
            ...
            order_result = await self.grvt_client.place_close_order(
                self.grvt_config.contract_id, reverse_quantity, target_price, direction
            )
            if not order_result.success:
                last_error = order_result.error_message or "Unknown error"
                self.logger.log(
                    f"{leg_name} | Attempt {attempt} failed to place GRVT close: {last_error}",
                    "ERROR",
                )
                if attempt >= self.config.max_retries:
                    raise RuntimeError(
                        f"{leg_name} | Failed to place GRVT close after {attempt} attempts: {last_error}"
                    )
                continue

            if not order_result.order_id:
                raise RuntimeError(f"{leg_name} | GRVT close order returned without an order id")
        ...
        self.logger.log(
            f"{leg_name} | GRVT reverse maker filled {fill_info.filled_size} @ {fill_info.price}",
            "INFO",
        )
        ...
        while time.time() < deadline:
            ...
            if order_info:
                ...
                if order_info.status in {"CANCELED", "REJECTED", "EXPIRED"}:
                    raise RuntimeError(f"{leg_name} | GRVT order ended with status {order_info.status}")
            await asyncio.sleep(self.config.poll_interval)

        await self._attempt_cancel_grvt(order_id, leg_name, last_status)

        final_info = await self.grvt_client.get_order_info(order_id=order_id)
        if final_info and final_info.status == "FILLED":
            self.logger.log(
                f"{leg_name} | GRVT order {order_id} filled while waiting for cancellation.",
                "INFO",
            )
            return final_info

        raise TimeoutError(f"{leg_name} | GRVT order {order_id} not filled within timeout")
    ...
    async def _attempt_cancel_grvt(
        ...
        try:
            ...
            if not result.success:
                self.logger.log(
                    f"{leg_name} | Failed to cancel GRVT order {order_id}: {result.error_message}",
                    "ERROR",
                )
        except Exception as exc:  # pragma: no cover - defensive logging
            self.logger.log(
                f"{leg_name} | Exception canceling GRVT order {order_id}: {exc} (last status={last_status})",
                "ERROR",
            )
    ...
    async def _calculate_grvt_maker_price(self, direction: str) -> Decimal:
        if not self.grvt_client:
            raise RuntimeError("GRVT client is not connected")

        best_bid, best_ask = await self.grvt_client.fetch_bbo_prices(self.grvt_config.contract_id)
        tick = self.grvt_config.tick_size if self.grvt_config.tick_size > 0 else Decimal("0")
        ...
        if direction == "sell":
            if best_ask > 0:
                ...
                price = None
            if price is not None and best_bid > 0 and price <= best_bid:
                price = best_bid + minimal_step
        else:
            if best_bid > 0:
                price = best_bid
                ...
                price = None
            if price is not None and best_ask > 0 and price >= best_ask:
                price = best_ask - minimal_step

        if price is None or price <= 0:
            raise ValueError("Unable to determine GRVT maker price from market data")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Execute a single GRVT–Lighter hedging cycle using existing exchange adapters",
    )
    parser.add_argument("--grvt-ticker", required=True, help="Symbol on GRVT (e.g., ETH)")
    parser.add_argument(
        "--lighter-ticker",
        required=True,
    ...
    parser.add_argument(
        "--grvt-quantity",
        type=_decimal_type,
        help="Override quantity for GRVT maker legs (defaults to --quantity)",
    )
    parser.add_argument(
        "--lighter-quantity",
        type=_decimal_type,
    ...
    parser.add_argument(
        ...
        choices=["buy", "sell"],
        default="buy",
        help="Initial maker direction on GRVT",
    )
    parser.add_argument(
        "--take-profit",
        type=_decimal_type,
        default=Decimal("0"),
        help="Reserved for compatibility (currently unused in reverse GRVT leg)",
    )
    parser.add_argument(
        "--slippage",
        type=_decimal_type,
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Maximum seconds to wait for GRVT fills before canceling",
    )
    parser.add_argument(
        "--lighter-max-wait",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Maximum number of retries for GRVT maker orders before aborting",
    )
    parser.add_argument(
        "--retry-delay",
        type=float,
        default=5.0,
        help="Delay in seconds before retrying a maker order",
    )
    parser.add_argument(
        "--cycles",
        type=int,
...
def _calculate_pair_metrics(leg_a: LegResult, leg_b: LegResult) -> Tuple[Decimal, Decimal]:
    if leg_a.exchange == leg_b.exchange:
        raise ValueError("Pair legs must come from different exchanges to compute PnL")

    pnl = (leg_b.price - leg_a.price) * leg_a.quantity
    volume = leg_a.price * leg_a.quantity + leg_b.price * leg_b.quantity
    return pnl, volume
...
async def _async_main(args: argparse.Namespace) -> None:
    ...
    config = CycleConfig(
        grvt_ticker=args.grvt_ticker,
        lighter_ticker=args.lighter_ticker,
        quantity=args.quantity,
        grvt_quantity=args.grvt_quantity if args.grvt_quantity is not None else args.quantity,
