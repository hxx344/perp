...
- `--cycle-delay`: Additional pause (seconds) applied after each cycle. The executor now enforces a minimum cadence of 5 seconds between cycle starts, so this value is added on top of that floor.
...
- `--coordinator-url` / `--coordinator-vps-id`: Enable the generic grid-bot coordinator. See [docs/grid_bot_coordinator.md](docs/grid_bot_coordinator.md) for deployment and authentication.
